...
class GUI(Tk):
    ...
    def set_speed_limit(self, speed):
        ...
        self._canvas.itemconfig(
            self._speed_limit_sym._id,
            image=self._speed_limit_sym._image_tk)
    ...
    def _update_map(self):
        ...
        if self._map_id is None:
            self._map_id = self._canvas.create_image(
                (275, 475), image=self._map_tk)
        else:
        ...
        if self._marker is not None:
            ...
            self._marker_id = self._canvas.create_polygon(
                self._marker, fill="blue")
